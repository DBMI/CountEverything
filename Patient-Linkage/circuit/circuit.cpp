...
*/
	else if ( name == "social-net" )
	{
 	...
 	else if(name=="matrixLib")
  	{
  		pCircuit = new MatrixLib();	
  	}
  	else if(name=="fixedPoint")
  	{
  		pCircuit=new FixPoint();
  	}
  	else if(name=="patientLnk")
  	{
		pCircuit=new PatientLnk();
  	}
	else if(name=="Multiparty")
  	{
		pCircuit=new Multiparty();
  	}
   else if(name=="Multipartya")
  	{
		pCircuit=new Multipartya();
  	}
	if(pCircuit)  
	{
		BOOL b = pCircuit->Create(nParties, params);
		cout<<"--------------------------created circuit---------------------\n";
		if( !b )
		{
			delete pCircuit;
			return NULL;
